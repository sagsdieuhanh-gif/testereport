...
self.addEventListener("message",event=>{
  ...
  if(event.data&&event.data.type==="GET_BUILD"&&event.ports&&event.ports[0])event.ports[0].postMessage({build:BUILD});
});
self.addEventListener("fetch",event=>{
  ...
  const isFresh=isNavigation||["/version.json","/v2-version.json","/firebase-config.js","/index.html","/daily-roster.js","/flight-registry-v2.js","/ai-crosscheck.js","/document-scanner.js","/admin-builder.js","/ac-limits.js"].some(x=>url.pathname.endsWith(x));
