/* E-REPORT SAGS · FLIGHT WORKSPACE · V1.88
...
const SCHEMA=2;
const DB_ROOT="flight_workspace_v188";
const ROSTER_MANIFEST="roster_manifests";
const ACTIVE_KEY="sagsFlightWorkspaceActiveV188";
const HOME_KEY="sagsFlightWorkspaceHomeShownV188";
const LOCAL_DAY_KEY="sagsFlightWorkspaceLocalDayV188";
...
function installUi(){
  if(!document.getElementById("fw188Style")){
    const style=document.createElement("style");style.id="fw188Style";style.textContent=`
...
`;
  ...
  if(!document.getElementById("fwHome")){
    const h=document.createElement("div");h.id="fwHome";h.innerHTML=`<div class="fwShell"><div class="fwTop"><div><div class="fwTitle">✈️ CHUYẾN BAY HÔM NAY <span class="fwVersion">V1.88</span></div><div class="fwSub" id="fwHeaderSub"></div><div class="fwDebug">Flight Workspace build ${BUILD}</div></div><div class="fwTopActions"><button class="fwBtn green" id="fwCreateBtn" onclick="SAGSFlightWorkspace.openCreate()">＋ TẠO CHUYẾN</button><button class="fwBtn orange" id="fwRepairBtn" onclick="SAGSFlightWorkspace.openRepair()" disabled>🔀 ĐỔI CẶP</button><button class="fwBtn gray" onclick="SAGSFlightWorkspace.openGuide()">HDSD</button><button class="fwBtn gray" onclick="SAGSFlightWorkspace.close()">ĐÓNG</button></div></div><div class="fwBody"><div class="fwList" id="fwList"></div><div class="fwDetail" id="fwDetail"></div></div></div>`;document.body.appendChild(h);
  }
...
function ensureEntry(){
  ...
  else if(!document.getElementById("fwFloating")){const f=document.createElement("button");f.id="fwFloating";f.className="fwFloating";f.textContent="✈️ CHUYẾN BAY · V1.88";f.onclick=open;document.body.appendChild(f);}
}
...
function start(){
  ...
  try{const base=root.applyRoleUI;if(typeof base==="function"&&!base.__fw188){const wrap=function(){const r=base.apply(this,arguments);setTimeout(maybeOpenHome,80);return r;};wrap.__fw188=true;root.applyRoleUI=wrap;}}catch(_){}
  try{const mo=new MutationObserver(()=>{if(role()){ensureEntry();maybeOpenHome();}});mo.observe(document.body,{childList:true,subtree:true});root.__fw188Observer=mo;}catch(_){}
...
root.__SAGS_FW188_TEST__={planRepair,tokens,fmtTime,isoDate,flightNo,station};
